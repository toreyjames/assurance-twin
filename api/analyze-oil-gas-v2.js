...
const normalizeDataset = (rows) => rows.map((row) => {
  ...
  return {
    ...
    device_type: String(norm.device_type ?? norm.type ?? norm.asset_type ?? norm.instrument_type ?? '').trim(),
...
function performFlexibleMatching(engineering, discovered, options = {}) {
  const {
    demoMode = false,  // Set to true only for demos with forced coverage
    matchStrategies = ['tag_id', 'ip_address', 'hostname', 'mac_address'],
    minCoverage = null, // Only used if demoMode = true
    maxCoverage = null  // Only used if demoMode = true
  } = options
  ...
  console.log(`[MATCHING] Final: ${matchedAssets.length}/${engineeringAssets} matched (${coveragePercentage}%)`)
  console.log(`[MATCHING] Blind spots: ${blindSpots.length}, Orphans: ${orphanAssets.length}`)
...
// Oil & Gas Industry-Specific Canonizer - PRODUCTION VERSION (No Subjective Criticality)
export default async function handler(req, res) {
  try {
    console.log('Analyze Oil & Gas V2 (Production) called with method:', req.method)
    ...
    const matchingOptions = demoMode ? {
      ...
      demoMode: false,
      matchStrategies: ['ip_address', 'hostname', 'mac_address', 'tag_id'] // IP first in production
    }
    
    const otDiscoveryAnalysis = performFlexibleMatching(eng, otDiscovery, matchingOptions)

    // Calculate objective KPIs (NO subjective criticality)
    ...
    // Process Unit Distribution (objective counts only)
    const processUnitDistribution = eng.reduce((acc, asset) => {
    ...
    return res.status(200).json({
      ...
      suggestedMatches: otDiscoveryAnalysis.unmatchedEngineering.slice(0, 10).map(engAsset => {
        // Find potential matches from orphan assets
